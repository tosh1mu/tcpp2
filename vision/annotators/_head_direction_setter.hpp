/*
 * head_direction_setter.hpp
 *
 *  Created on: 2012/11/20
 *      Author: takahashi
 */

#ifndef HEAD_DIRECTION_SETTER_HPP_
#define HEAD_DIRECTION_SETTER_HPP_
...
class HeadDirectionSetter {
public:
	HeadDirectionSetter( const cv::Mat& image, HumanImageLabels& labels, HumanImageLabels reference_labels = HumanImageLabels() ):
		labels_(labels), selected_label_(reference_labels.head_direction()), current_label_(labels.head_direction()),
		...
		unselected_color_(255, 0, 0), selected_color_(0, 0, 255), fixed_color_(0, 255, 0)
	{
		...
		std::string label_text = "Head Direc.";
	...
	void Fix() {
		if(selected_label_ != -1) {
			...
			selected_label_ = -1;
			SelectLabel(selected_label_);
			labels_.set_head_direction(current_label_);
			labels_.Save();
		}
	}
...
void HeadDirectionSet( int event, int x, int y, int flags, void* _head_direction_setter ) {
	HeadDirectionSetter<direction_num>* head_direction_setter = reinterpret_cast<HeadDirectionSetter<direction_num>*>(_head_direction_setter);

	switch(event) {
	case cv::EVENT_LBUTTONDOWN:
		head_direction_setter->SetLabel( x, y );
		break;
	}
}
...
#endif /* HEAD_DIRECTION_SETTER_HPP_ */
